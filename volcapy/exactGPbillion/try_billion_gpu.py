...
def train(train_x, train_y, n_devices, output_device,
        checkpoint_size, preconditioner_size, n_training_iter):
...
def find_best_gpu_setting(train_x, train_y, n_devices, output_device,
    ...
    for checkpoint_size in settings:
        print('Number of devices: {} -- Kernel partition size: {}'.format(
                n_devices, checkpoint_size))
        try:
            # Try a full forward and backward pass with this setting to check
            # memory usage.
...
checkpoint_size = find_best_gpu_setting(train_x, train_y, n_devices=n_devices,
        output_device=output_device,
...
with (torch.no_grad(), gpytorch.settings.fast_pred_var(),
    ...
    # Make predictions on a small number of test points to get the test time
    # caches computed.
    latent_pred = model(test_x[:2, :])
    del latent_pred
    # We don't care about these predictions, we really just want the caches.
