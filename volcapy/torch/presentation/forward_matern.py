...
# IMPORTANT
###########
out_folder = "/idiap/temp/ctravelletti/out/matern/"
...
def compute_K_d(lambda0, F):
    ...
    for i, x in enumerate(torch.chunk(cells_coords, chunks=200, dim=0)):
        ...
        tot = torch.cat((
                tot,
                torch.matmul((1 - inv_lambda2 * d) * torch.exp(inv_lambda2 * d)
                    , F.t())))
...
class SquaredExpModel(torch.nn.Module):
    ...
    def forward_model(self, cov_pushfwd, sigma0, m0=0.1, concentrate=False):
        logger.debug("GPU used before forward pass: {} Gb.".format(
                torch.cuda.memory_allocated(device)/1e9))
        ...
        self.mu0_d = m0 * self.mu0_d_stripped
        self.mu0_model = m0 * torch.ones((n_model, 1))
        ...
        return (log_likelihood, m_posterior_model, m_posterior_d)
...
# Compute the covariance pushforward.
cov_pushfwd = compute_K_d(lambda0, F_gpu)
        
# Once finished, run a forward pass.
log_likelihood, m_posterior_model, d_post = model.forward_model(
        cov_pushfwd, sigma0, concentrate=True)

# Compute train error.
train_error = torch.sqrt(torch.mean(
    (model.d_obs - d_post)**2))
...
np.save(os.path.join(out_folder, "m_post.npy"), m_post)
