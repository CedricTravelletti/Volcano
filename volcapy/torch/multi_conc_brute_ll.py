...
class SquaredExpModel(torch.nn.Module):
    ...
    def forward(self, distance_mesh):
        ...
        inv_lambda2 = (- 1/(2 * self.length_scale.pow(2)))
        pushforward_cov = torch.cat(
                [torch.matmul(torch.exp(torch.mul(inv_lambda2, x)), F.t())
                    for i, x in enumerate(torch.chunk(distance_mesh, chunks=20, dim=0))],
                dim=0)
