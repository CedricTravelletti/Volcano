...
class SquaredExpModel(torch.nn.Module):
	...
	m_ML = (torch.mm(
		torch.inverse(F.t() inversion_operator F) * 
		torch.mm(F.t(), torch.mm(inversion_operator, self.d_obs))) log_likelyhood = log det inversion_operator + (d_obs - F m_ML).t() 
				* inversion_operator * (d_obs - F m_ML)
